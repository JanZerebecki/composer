...
class GitDownloader extends VcsDownloader
{
    ...
    private function verifyCommit($path, $gitReference, $commitId = null) {
        ...
        exec( 'git --git-dir=' . escapeshellarg($path.'/.git') . ' rev-parse --revs-only --symbolic-full-name ' . escapeshellarg($gitReference) . ' -- 2>&1', $outputParse, $resultParse );
        if ($this->io->isDebug()) {
        ...
        $type = 'commit';
        if (0 === $resultParse && count($outputParse) === 1 && $outputParse[0] === 'refs/tags/' . $gitReference) {
            $type = 'tag';
        }
        ...
        if ($commitId !== null) {
            $output = array();
            $result = 1000;
            exec( 'git --git-dir=' . escapeshellarg($path.'/.git') . ' log --format=format:%H -1 ' . escapeshellarg($outputParse[0]) . ' -- 2>&1', $output, $result);
            if ($result !== 0 || count($output) > 1 || $output[0] !== $commitId) {
                $this->io->writeError("git log for $outputParse[0] unexpectedly returned $result and the following output:");
                $this->io->writeError($output);
                $this->io->writeError("Expected return 0 and commit id:");
                $this->io->writeError($commitId);
                return false;
            }
        }
        ...
        $output = array();
        $result = 1000;
        exec( 'git --git-dir=' . escapeshellarg($path.'/.git') . ' verify-' . $type . ' ' . escapeshellarg($outputParse[0]) . ' 2>&1', $output, $result );
        if ($result !== 0) {
            $this->io->writeError("git verify-$type of $outputParse[0] returned error $result. Output follows:");
            $this->io->writeError($output);
            return false;
        }
        ...
        $output = implode("\n", $output);
        $matches = null;
        $pattern = '/(?m)(?:^gpg: (?:Signature made .*$|\s+using \S+ key 0x.*$|Good signature from .*$|\s+aka .*$)|^(?:Primary key|     Subkey) fingerprint: (?P<fingerprint>[ 0-9A-F]{50})$)/';
        $result = preg_match_all($pattern, $output, $matches);
        if ($lineCount < 4 || $result !== $lineCount) {
            $this->io->writeError("The output of git verify-$type of $outputParse[0] didn't match a successful verification. Output follows:");
            $this->io->writeError($output);
            return false;
        }
        ...
        if ($this->io->isDebug()) {
            $this->io->writeError("git verify-$type of $outputParse[0] aka $gitReference for $matchedFingerprint was successful. Output follows:");
            $this->io->writeError($output);
            if ($commitId !== null) {
                $this->io->writeError("The reference also matches the commit id: $commitId");
            }
        }
        return true;
    }
    ...
    protected function updateToCommit($path, $reference, $branch, $date)
    {
        ...
        if (!preg_match('{^[a-f0-9]{40}$}', $reference)
            ...
            $prefixedBranch = 'composer/'.$reference;
            if ($this->verifyCommit($path, $prefixedBranch)) {
                $command = sprintf('git checkout -B %s %s -- && git reset --hard %2$s --', ProcessExecutor::escape($branch), ProcessExecutor::escape($prefixedBranch));
                if (0 === $this->process->execute($command, $output, $path)) {
                    return;
                }
            }
        ...
        if (preg_match('{^[a-f0-9]{40}$}', $reference)) {
            ...
            if ($this->verifyCommit($path, $branch, $reference)) {
            ...
            if ($this->verifyCommit($path, $prefixedBranch, $reference)) {
        ...
        if (!$this->verifyCommit($path, $gitRef)) {
